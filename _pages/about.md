---
permalink: /
title: "Tung Sum Thomas Kwok"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I am a first-year PhD candidate at University of California Los Angeles and a member of the Trustworthy AI Lab advised by Professor Guang Cheng. My research interests lie on the large language models (`LLM's), world models and tabular data modeling. Happy to connect and discuss further! 

## 📢 News
- Feb 2026: Paper accepted to the Pacific-Asia Conference on Knowledge Discovery and Data Mining
- Jan 2026: Paper accepted to The ACM Web Conference 2026.
- Jan 2026: VeritasFi won Best Paper at AAAI 2026 Workshop on Agentic AI in Financial Services.
- Sep 2025: Began PhD studies at UCLA.
- July 2025: Paper accepted to the KDD' 25 Workshop on Evaluation and Trustworthiness of Agentic and Generative AI Models.
- May 2025: Paper accepted to The Conference on Information and Knowledge Management 2025.
- Feb 2025: Paper accepted to ICDE' 25 Data Engineering Meets Large Language Models: Challenges and Opportunities Workshop.
- Jan 2025: Graduated from Master's of Mathematical Finance and Financial Technology program at Boston University.


## 📄 Selected Publications ([full list](https://scholar.google.com/citations?user=3craQtgAAAAJ&hl=en))

<div class="pub-cards">
  <article class="pub-card">
    <div class="pub-thumb">
      <img src="/images/retab.png" alt="RE-Tab Illustration figure">
      <span class="badge badge-preprint">Preprint</span>
    </div>
    <div class="pub-body">
      <h3 class="pub-title">Enhancing TableQA through Verifiable Reasoning Trace Reward</h3>
      <p class="pub-authors"><strong>Tung Sum Thomas Kwok*</strong>, Xinyu Wang*, Hengzhi He, Xiaofeng Lin, Peng Lu, Liheng Ma, Chunhe Wang, Ying Nian Wu, Lei Ding, Guang Cheng</p>
      <p class="pub-highlight">Structured table reasoning with processed reward model supervision</p>
      <div class="pub-links">
        <a class="pill" href="https://arxiv.org/pdf/2601.22530">PDF</a>
      </div>
    </div>
  </article>

  <article class="pub-card">
    <div class="pub-thumb">
      <img src="/images/VeritasFi.png" alt="TableQA figure">
      <span class="badge badge-venue">WWW 2026</span>
    </div>
    <div class="pub-body">
      <h3 class="pub-title">VeritasFi: Multi-tiered RAG for Multimodal Financial QA</h3>
      <p class="pub-authors">Zhenghan Tai, Hanwei Wu, Qingchen Hu, Jijun Chi, Hailin He, Lei Ding, <strong>Tung Sum Thomas Kwok</strong>, et al.</p>
      <p class="pub-highlight">Hybrid RAG framework with multi-modal preprocessing and retrieval components for financial documents</p>
      <div class="pub-links">
        <a class="pill" href="https://arxiv.org/pdf/2510.10828">PDF</a>
      </div>
    </div>
  </article>

  <article class="pub-card">
    <div class="pub-thumb">
      <img src="/images/finsage.png" alt="FinSage figure">
      <span class="badge badge-venue">CIKM 2025</span>
    </div>
    <div class="pub-body">
      <h3 class="pub-title">VeritasFi: Multi-tiered RAG for Multimodal Financial QA</h3>
      <p class="pub-authors">Xinyu Wang, Jijun Chi, Zhenghan Tai, <strong>Tung Sum Thomas Kwok</strong>, et al.</p>
      <p class="pub-highlight">A multi-path sparse-dense retrieval system augmented with query expansion, metadata-aware semantic search and a domain-specialized re-ranking module fine-tuned via Direct Preference Optimization</p>
      <div class="pub-links">
        <a class="pill" href="https://dl.acm.org/doi/pdf/10.1145/3746252.3761587">PDF</a>
      </div>
    </div>
  </article>

  <article class="pub-card">
    <div class="pub-thumb">
      <img src="/images/greater.png" alt="GreatER figure">
      <span class="badge badge-venue">ICDE 2025 Workshop</span>
    </div>
    <div class="pub-body">
      <h3 class="pub-title">GReaTER: Generate Realistic Tabular data after data Enhancement and Reduction</h3>
      <p class="pub-authors"><strong>Tung Sum Thomas Kwok</strong>, Chi-Hua Wang, Guang Cheng.</p>
      <p class="pub-highlight">Tabular data synthesis with Semantic Enhancements</p>
      <div class="pub-links">
        <a class="pill" href="https://arxiv.org/pdf/2503.15564">PDF</a>
      </div>
    </div>
  </article>
</div>


## 💼 Selected Experience
- Adaptive Investments — Quantitative Research Intern (2024): financial RL, option and bond pricings
- Hammer Capital — Full-time Investment Analyst (2021-2023): leveraged notes origination, debt restructuring advisory


## ✨ Honors & Awards
- AAAI 2026 Workshop Best Paper Award (2026)
- OpenAI Researcher Access Program Award (2025)
- ICAIF 2024 Workshop Best Presentation Award (2024)
- Highest Honor Graduation, Boston University (2025)
- BU MSMFT Dean’s Achievement Scholarship (2023)
- CUHK Dean’s List Award (2020)


## 🌍 Academic Services
- Program committee/reviewer: ICML 2026
- Conference student volunteer: ICLR 2026
- Panelist: ACM@UCLA 2025
- Teaching: Guest lecturer for STATS161/261 (Winter 2025, Fall 2025), STATS163/263 (Winter 2026); course assistant/grader for STATS10, STATS163/263


## 🧩 Miscellaneous
Former track athlete (10th in Hong Kong Junior Triple Jump Ranking 2016 - 2017; 6th in USFHK 2018 - 2019). Also into poker, Chinese chess and basketball. 
